...
data(data),GF(GF), model(env), f(data.m, std::vector<GRBVar>(data.n)),
root(data.n), arc(GF.arc_set.size()), arc_f(data.m,std::vector<GRBVar>(GF.arc_set.size()))
{
    ...
    ////tree constraints TODO:SOSCONSTRAINTS
    //////one root
